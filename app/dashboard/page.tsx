...
import { ParkingCircle, Car, Clock, TrendingUp, Calendar, RefreshCw } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const stats = [
    ...
    { title: "Gastos este mes", value: "S/ 184.00", description: "Este mes", icon: TrendingUp, trend: "+12.5% desde el mes pasado", iconColor: "text-emerald-600 dark:text-emerald-400", iconBg: "bg-emerald-100 dark:bg-emerald-900/30", trendColor: "text-green-600 dark:text-green-400" },
    ...
    { title: "Duración Promedio", value: "4.5h", description: "Por sesión de estacionamiento", icon: Clock, trend: "Estable", iconColor: "text-amber-600 dark:text-amber-400", iconBg: "bg-amber-100 dark:bg-amber-900/30", trendColor: "text-muted-foreground" },
  ];

  const reservationsChartData = [
    { date: "2025-11-01", reservas: 1, ingresos: 10 },
    { date: "2025-11-02", reservas: 2, ingresos: 14 },
    { date: "2025-11-03", reservas: 1, ingresos: 9 },
    { date: "2025-11-04", reservas: 1, ingresos: 16 },
    { date: "2025-11-05", reservas: 1, ingresos: 15 },
    { date: "2025-11-06", reservas: 3, ingresos: 13 },
    { date: "2025-11-07", reservas: 1, ingresos: 18 },
    { date: "2025-11-08", reservas: 1, ingresos: 16 },
    { date: "2025-11-09", reservas: 0, ingresos: 0 },
    { date: "2025-11-10", reservas: 1, ingresos: 13},
    { date: "2025-11-11", reservas: 1, ingresos: 17},
    { date: "2025-11-12", reservas: 1, ingresos: 15 },
    { date: "2025-11-13", reservas: 1, ingresos: 13 },
    { date: "2025-11-14", reservas: 1, ingresos: 15 },
  ];
  ...
  const occupancyLineData = [
    { month: "Junio", ocupacion: 20 },
    { month: "Julio", ocupacion: 15 },
    { month: "Agosto", ocupacion: 23 },
    { month: "Septiembre", ocupacion: 15 },
    { month: "Octubre", ocupacion: 16 },
    { month: "Noviembre", ocupacion: 25 },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat, index) => {
          ...
          return (
            <div key={stat.title} className={`p-6 rounded-xl backdrop-blur-lg border ${config.borderColor} bg-linear-to-tr from-card/60 to-card/40 dark:from-card/80 dark:to-card/60 shadow-lg ${config.shadowColor} transition-all duration-300 ease-out cursor-pointer hover:bg-linear-to-tr ${config.gradientFrom} hover:to-card/40 dark:hover:to-card/60 group relative overflow-hidden`}>
              <div className={`absolute inset-0 bg-linear-to-tr from-transparent ${config.shimmerColor} to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700 ease-out`} />
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <Card className="col-span-2 py-0 transition-all duration-300 hover:shadow-lg border-border/50">
          <CardHeader className="flex flex-col items-stretch border-b p-0! sm:flex-row bg-linear-to-r from-background to-muted/20">
            ...
            <div className="flex">
              {(["reservas", "ingresos"] as const).map((key) => (
                <button key={key} type="button" data-active={activeChart === key} className="data-[active=true]:bg-muted/50 relative z-30 flex flex-1 flex-col justify-center gap-1 border-t px-6 py-4 text-left even:border-l sm:border-t-0 sm:border-l sm:px-8 sm:py-6" onClick={() => setActiveChart(key)}>
        ...
        <Card className="flex flex-col transition-all duration-300 hover:shadow-lg border-border/50">
          <CardHeader className="items-center pb-0 bg-linear-to-br from-background to-muted/20">
        ...
        <Card className="col-span-2 transition-all duration-300 hover:shadow-lg border-border/50">
          <CardHeader className="bg-linear-to-r from-background to-muted/20">
